...
int main(int argc, char* argv[]){
	...
	sprintf(AIDAfile,"$NP1306DIR/AIDA/R%04d_%d_hitbuild_ion_beta.root",runA,runA_sub);
	sprintf(outfile,"$NP1306DIR/BeamGe_AIDA/GA_G%04d_A%04d_%d.root",runG,runA,runA_sub);
	}else if (argc==3){
        ...
        sprintf(gamfile,"$NP1306DIR/EURICA_exp/run_%04d.root",runG);
        sprintf(calfile,"$NP1306DIR/Beam_Gamma/152Eu_eff_1000Hz_0001_calibdata.root");
        sprintf(AIDAfile,"$NP1306DIR/AIDA/R%04d_hitbuild_ion_beta.root",runA);
        sprintf(outfile,"$NP1306DIR/BeamGe_AIDA/GA_G%04d_A%04d.root",runG,runA);
...
//	TTree* tree = new TTree("tree","tree");
	TTree* tree2 = new TTree("beta_ion","beta_ion");
	
	BeamGe_AIDA outtree;
	outtree.TreeBranchBetaGam(tree2);

	std::cout<<mtse.size()<<" events for beta_gamma events"<<std::endl;
	std::cout<<mtsg.size()<<" events for ion_beam events"<<std::endl;


	// The timestamps are in timestamp unit.	10ns/1stamp
	
	Long64_t beta_gam_tsL = 1300; 
	Long64_t beta_gam_tsU = 1800; // 100ns window
